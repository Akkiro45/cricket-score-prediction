...
def predict():
	new_prediction = reg.predict(sc.transform(np.array([[int(batTeam.get()),int(ballTeam.get()),int(score.get()),int(wikts.get()),int(over.get()),int(strikersRuns.get()),int(nonStrikersRuns.get())]])))
